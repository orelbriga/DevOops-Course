...
def checkWin():
    with open(r'C:\Users\orel.briga\Downloads\DevOps Course\Project_file.txt', 'r') as mainFile:
        ...
        '''In case the player choose a position that is higher then last letter in the file,
           we first force the seek to point the real last letter and then we move one step backwards
        ...
        while not hitTheTreasure:
            try:
                ...
                if pointer == 1:
                    ...
                    counter += 1
                    checkWin()
                    move -= 1  #returing the pointer one step behind after reading the letter
